# Structured Session Naming — Implementation Plan (Approved)

> **Reviewed:** 2026-03-04 — APPROVED with minor fixes incorporated. See `plan-review.md`.
...
1. **Test imports**: Tasks 2–5 show per-describe imports for readability, but the implementer
   must consolidate all imports at the top of the test file. Ensure `type ExecFn` is imported
   alongside each resolver (Task 8 shows the correct pattern).

2. **Task 12 integration tests**: The test outlines are intentionally skeletal — the implementer
   should flesh out each test with full mock `exec` functions and concrete assertions matching
   the shaping examples table. Key scenarios to cover: all four shaping examples, plus the
   fallback path where all resolvers + description LLM fail.

3. **`project.org`/`area.org` fallback**: Deferred from `resolveSubfolder` — git-root detection
   covers the majority case. Non-git directories will produce description-only or fallback names.
   Can be added in a future wave if needed.

4. **Duplicate `--show-toplevel` calls**: `detectWorktree` and `resolveSubfolder` both call
   `git rev-parse --show-toplevel`. Minor inefficiency — they run in parallel so no latency
   impact. Not worth optimizing now.
