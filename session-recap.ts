/**
 * Session recap for Pi.
 *
 * Forked from tmustier/pi-extensions session-recap:
 * https://github.com/tmustier/pi-extensions/tree/main/session-recap
 * Source commit: 31d0cf00f64a4fd6422ee3ff3a349db9982f2f2e
...
function hasMeaningfulActivity(entries: Entry[]): boolean {
	...
	const tail =
		lastUserIdx >= 0 ? entries.slice(lastUserIdx + 1) : entries;
...
function showRecap(ctx: ExtensionContext, recap: string) {
	...
	ctx.ui.setWidget(WIDGET_KEY, [header, theme.fg("dim", recap)], {
...
export default function (pi: ExtensionAPI) {
	pi.registerFlag("recap-idle-seconds", {
		description:
			"Seconds after turn_end before the session recap is generated",
		type: "string",
		default: String(DEFAULT_IDLE_SECONDS),
	});
	pi.registerFlag("recap-focus-min-seconds", {
		description:
			"Minimum focus-out duration (seconds) before showing a recap on refocus",
		type: "string",
		default: String(DEFAULT_FOCUS_MIN_SECONDS),
	});
	pi.registerFlag("recap-disable-focus", {
		description:
			"Disable DECSET ?1004 focus reporting (idle fallback still runs)",
		type: "boolean",
		default: false,
	});
	pi.registerFlag("recap-during-active", {
		description:
			"Allow focus-triggered recaps while an agent turn is still running",
		type: "boolean",
		default: false,
	});
	...
	pi.registerFlag("recap-model", {
		description:
			"Override the default (active) model, e.g. anthropic/claude-sonnet-4-6",
		type: "string",
		default: "",
	});
	...
	const idleMs = (): number => {
		const n = Number(
			pi.getFlag("recap-idle-seconds") ?? DEFAULT_IDLE_SECONDS,
		);
		return (
			Math.max(5, Number.isFinite(n) ? n : DEFAULT_IDLE_SECONDS) * 1000
		);
	};
	const focusMinMs = (): number => {
		const n = Number(
			pi.getFlag("recap-focus-min-seconds") ??
				DEFAULT_FOCUS_MIN_SECONDS,
		);
		return (
			Math.max(0, Number.isFinite(n) ? n : DEFAULT_FOCUS_MIN_SECONDS) *
			1000
		);
	};
	const isDisabled = (): boolean =>
		Boolean(pi.getFlag("recap-disable"));
	const isFocusDisabled = (): boolean =>
		Boolean(pi.getFlag("recap-disable-focus"));
	const allowDuringActive = (): boolean =>
		Boolean(pi.getFlag("recap-during-active"));
	...
	const generateAndShow = async (
		...
		const showStatus =
			opts.reason !== "resume" && opts.reason !== "focus";
		if (showStatus && ctx.hasUI)
			ctx.ui.setStatus(
				STATUS_KEY,
				ctx.ui.theme.fg("dim", "✦ drafting recap…"),
			);
